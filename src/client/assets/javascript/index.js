...
async function handleCreateRace() {
	...
	try{
		    const res=await createRace(store.player_id,store.track_id)
			store.race_id=res.ID
			//console.log(store.race_id)
			renderAt('#race', renderRaceStartView(res.Track))
		    await runCountdown()
...
function getRacers() {
	...
	let data;
	try{
		data=fetch(`${SERVER}/api/cars`,{
			method:'GET',
			...defaultFetchOpts(),
		}).then((res)=>res.json())
	}catch(error){
		console.log("Error in getting racers")
	}
	return data;
}
...
async function accelerate(id) {
	...
	return fetch(`${SERVER}/api/races/${id}/accelerate`, {
	  ...
	  .catch((error) => console.log("Error in Accelerate ",error));
}
